...
unit uFormat.Controller.Vcl;

interface

uses Vcl.StdCtrls, Classes, System.MaskUtils, uInterface.Mascaras.Vcl;
...
type
   ...
   public
     procedure MascaraEdit(value:TFormato);
   end;

    TMascarasUteis = class(TInterfacedObject,iMascarasEditCaption)
    private
      fValue:string;
    ...
    class function New:iMascarasEditCaption;
end;
...
function TMascarasUteis.SomenteNumero(str : string) : string;
var
    x : integer;
begin
    result:='';
    for x := 0 to Length(str)-1 do
        if (str.Chars[x] >='0') AND (str.Chars[x] <='9') then
            result  := Result + str.Chars[x];
end;

function TMascarasUteis.FormataValor(str : string) : string;
begin
    if Str = '' then
        Str := '0';
...
function TMascarasUteis.FormataPeso(str : string) : string;
begin
    if Str.IsEmpty then
        Str := '0';
...
function TMascarasUteis.Mask(Mascara, Str : string) : string;
var
    x, p : integer;
begin
    p := 0;
...
function TMascarasUteis.FormataIE(Num, UF: string): string;
var
    Mascara : string;
begin
    Mascara := '';
...
function TMascarasUteis.FormataData(str : string): string;
begin
    str := Copy(str, 1, 8);
...
procedure TMascarasUteis.Formatar(Obj: TObject; Formato : TFormato; Extra : string = '');
var
    texto : string;
begin
    ...
    begin
        ...
        begin
            ...
            TLabel(obj).caption := texto;
   //         TLabel(obj).CaretPosition := TEdit(obj).Text.Length;
...
constructor TMascarasUteis.create;
begin

end;

destructor TMascarasUteis.destroy;
begin

  inherited;
end;

class function TMascarasUteis.New: iMascarasEditCaption;
begin
 result :=self.create;
end;

procedure TEditHelpers.MascaraEdit(value:TFormato);
var
  fmascara:iMascarasEditCaption;
begin
  fmascara:=TMascarasUteis.new;
  fmascara.formatar(self,value);
end;
